    ...
    try {
      ...
      if (trend === 'UP' && ema200Value < ema100Value && ema100Value < ema50Value) {
        ...
        if (price < ema100Value) {
          this.logger.log('Uptrend: Harga lebih rendah dari EMA100, tidak mempertimbangkan buy...');
          return;  // Jika harga lebih rendah dari EMA100, tidak membuka posisi buy
        }
      ...
      if (trend === 'DOWN' && ema200Value > ema100Value && ema100Value > ema50Value) {
        ...
        if (price > ema100Value) {
          this.logger.log('Downtrend: Harga lebih tinggi dari EMA100, tidak mempertimbangkan sell...');
          return;  // Jika harga lebih tinggi dari EMA100, tidak membuka posisi sell
        }
