...
export class BotV5Service implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    this.scheduleOrderCheck(); // Jadwalkan pemeriksaan posisi setiap 1 menit
  }
  ...
  private scheduleOrderCheck() {
    setInterval(async () => {
      const significantPriceChange = 0.005; // Batas perubahan harga signifikan
      await this.closeProfitableOrders(significantPriceChange);
    }, 60000);
  }
}
